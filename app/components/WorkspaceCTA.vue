<template>
  <UPageCTA
    :title="ctaData?.title || 'Ready to Experience the Future of Work?'"
  ...
  >
    <template #default>
      <div class="flex flex-col sm:flex-row gap-4 justify-center mt-8">
        <UButton 
          ...
          color="white" 
          variant="solid"
          :icon="ctaData?.buttons?.primary?.icon || 'i-heroicons-calendar-days'"
        ...
        >
          {{ ctaData?.buttons?.primary?.text || 'Book Your Space Now' }}
        </UButton>
        <UButton 
          size="xl" 
          ...
          color="white" 
          variant="outline"
          :icon="ctaData?.buttons?.secondary?.icon || 'i-heroicons-phone'"
          :to="ctaData?.buttons?.secondary?.url || '/contact'"
        >
          {{ ctaData?.buttons?.secondary?.text || 'Schedule a Consultation' }}
        </UButton>
      </div>
    </template>
    ...
    <template #bottom>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          <div class="space-y-2">
            ...
            <p class="text-white/80 text-sm">
              {{ selectedLocationData?.title || 'ABB Technology Center' }}<br>
              {{ selectedLocationData?.address?.street || 'Affolternstrasse 44' }}<br>
              {{ selectedLocationData?.address?.city || 'Zurich' }}, {{ selectedLocationData?.address?.country || 'Switzerland' }}
            </p>
          </div>
          ...
          <div class="space-y-2">
            ...
            <p class="text-white/80 text-sm">
              {{ selectedLocationData?.operating_hours?.weekdays || 'Monday - Friday: 6:00 AM - 10:00 PM' }}<br>
              {{ selectedLocationData?.operating_hours?.weekends || 'Saturday - Sunday: 8:00 AM - 8:00 PM' }}<br>
              {{ selectedLocationData?.operating_hours?.access || '24/7 access available for members' }}
            </p>
          </div>
          ...
          <div class="space-y-2">
            ...
            <p class="text-white/80 text-sm">
              ...
              {{ selectedLocationData?.contact?.phone || '+41 43 317 71 11' }}<br>
      ...
      <NuxtImg 
        ...
        :alt="`${selectedLocationData?.title || 'Workspace'} Background`" 
...
<div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
  ...
  <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3 lg:gap-6">
    <div class="p-4 md:p-7 bg-gray-100/10 rounded-lg dark:bg-neutral-800">
...
const selectedLocationData = computed(() => {
  ...
  return branchesData.value.find((branch: any) => branch.location === selectedLocation.value)
})
